...
9. Don't give a `BaseSettings` field a `list[...]` type if you want a human-friendly (e.g. comma-separated) env var for it. pydantic-settings JSON-decodes any list/dict-typed field read from an env var *before* any `field_validator` runs — a custom "split on comma" validator is simply never reached, and a non-JSON value raises `SettingsError` at startup. If you want a delimited string in the env, type the field as `str` and expose a computed `@property` that parses it (see `CallbackSettings.allowed_hosts` / `allowed_hosts_list`) — only fall back to a real `list[...]` field if the env var is meant to be actual JSON.
10. When a test asserts on custom fields attached via a log call's `extra={...}` (with `caplog`), pyright rejects `record.your_field` — `logging.LogRecord` doesn't statically declare arbitrary extras, even though they're real attributes at runtime. Type the local as `record: Any = next(...)` at the point you pull it out of `caplog.records`, rather than suppressing each field access individually.
