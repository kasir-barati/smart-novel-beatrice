...
## Post-step-7 review-fix pass — 2026-08-28

`REVIEW.md` (a `requirements-code-reviewer` pass over the full `1b420bd..0eaef74` range) surfaced one real correctness bug and two documented-gap findings, all addressed in one pass since none required redoing an already-committed step's scope:

1. **Retry-classification gap** (`worker.py` `upload_job`): a `genUploadUrl` that fails allow-list re-validation raised `CallbackUrlNotAllowedError` uncaught, which `decide_retry`'s `_status_code_of` treated as "no status code" — the same branch as a genuine network error — so it was endlessly retryable instead of going straight to the DLQ. Fixed by giving `CallbackUrlNotAllowedError` a duck-typed `status_code = 400` class attribute, the same pattern `TtsProviderError` already used for `decide_retry` to recognize it as non-retryable. Added coverage at both tiers: a `retry__test.py` unit test on `decide_retry` directly, and a `handle_message`-level test asserting the DLQ routing end-to-end — the review specifically noted the existing suite only asserted `upload_job` raises, not what the caller did with it.
2. **Streaming AC not implemented**: both providers buffered the full response body (`response.content`) before writing to disk, despite step 4's AC saying "stream the audio file into a temp file." Fixed for real in `Qwen3TtsProvider` (`client.stream(...)` + `aiter_bytes()` into the file). `GeminiTtsProvider` cannot truly stream to disk — Cloud TTS's REST API wraps audio as base64 inside one JSON envelope, so the full body must be received and parsed before any audio bytes exist — documented that constraint directly in the method's docstring rather than pretending to satisfy an AC that provider's transport shape makes impossible.
3. **Worker never actually runs under the documented dev flow**: `docker compose up --build -d` only started the GraphQL server; `src/worker.py` needed a separate `make start_worker` a developer had to remember. Added a `worker` service to `compose.yml` (same image, `python -m src.worker` command, healthcheck disabled since the image's default HEALTHCHECK curls a GraphQL port the worker never opens).
4. **Nitpick fixed per reviewer's explicit ask**: a malformed queue message (bad JSON, or a body that fails `GenerateAudioJob` validation) propagated out of `message.process()` uncaught — no structured log, no DLQ entry, silently rejected by aio_pika's default behavior. Now caught explicitly in `handle_message`, logged with `error_code: "MALFORMED_MESSAGE"`, and sent straight to the DLQ (no `job` exists yet to report failure through a callback, and a malformed body fails identically on every redelivery so retry is never right).
5. **Nitpick explicitly declined per reviewer's own instruction**: `_attempt_of`'s `int(...)` cast on a non-numeric `x-attempt` header was flagged but marked "SKIP THIS. DO NOT ADDRESS IT." in `REVIEW.md` — left untouched, since only this codebase's own publisher ever writes to the queue.

Process note: this wasn't a `REQUIREMENTS.md` step, so `PROCESS.md`'s per-step loop doesn't literally apply, but running the same shape (fix, add tests at the right tier, lint/typecheck, commit, self-improve log entry) worked fine unmodified — a review-response pass is close enough to a step that no new process doc is needed for it.
